        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                if self.phase=='train' and not isRandom:
                     ...
                     #print(sample.shape, target.shape, coord.shape)
            else:
            ...
            #print(self.count)
            #print(sample.shape)
            return torch.from_numpy(sample), torch.from_numpy(label['0']), torch.from_numpy(label['1']), torch.from_numpy(label['2']), torch.from_numpy(label['3']), coord
        else:
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        #print(imgs, target, isScale, isRand)
        ...
        #print(start, crop_size, imgs.shape, target)
        crop = imgs[:,
        ...
        #print(crop.shape, target.shape, coord.shape)
        #print(scale)
        return crop, target, bboxes, coord
    
class LabelMapping(object):
    def __init__(self, config, phase):
        self.stride_sets = np.array(config['stride_sets'])
    ...
    def __call__(self, input_size, target, bboxes):
        #print(input_size, target)
        #print(target)
        strides = self.stride_sets
        ...
        for index, stride in enumerate(strides):
            ...
            label = -1 * np.ones(output_size + [5], np.float32)
            #print(label.shape)
            ...
            if len(iz) == 0:
                ...
                dz = (target[0] - oz[pos[0]]) / anchor
                dh = (target[1] - oh[pos[1]]) / anchor
                dw = (target[2] - ow[pos[2]]) / anchor
                dd = np.log(target[3] / anchor)
                label[pos[0], pos[1], pos[2], :] = [1, dz, dh, dw, dd]
                #print([1, dz, dh, dw, dd])
                #pos.append(index)
                flag = False
            else:
                for idx in range(len(iz)):
                    #idx = random.sample(range(len(iz)), 1)[0]
                    pos = [iz[idx], ih[idx], iw[idx]]
            #print(pos)
                    dz = (target[0] - oz[pos[0]]) / anchor
                    dh = (target[1] - oh[pos[1]]) / anchor
                    dw = (target[2] - ow[pos[2]]) / anchor
...
def select_samples(bbox, anchor, th, oz, oh, ow):
    ...
    #print('min', min_overlap, 'max', max_overlap)

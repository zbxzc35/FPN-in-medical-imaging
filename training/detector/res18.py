...
config['anchors'] = [6., 12., 24., 36.]
config['chanel'] = 1
config['crop_size'] = [128, 128, 128]
config['stride_sets'] = [2, 4, 8, 16]
config['stride'] = 4
...
class Net(nn.Module):
    def __init__(self):
            ...
            for j in range(num_blocks_back[i]):
                if j == 0:
                    if i==1:
                        ...
                        addition = 0
                    blocks.append(PostRes(self.featureNum_back[i] + addition, self.featureNum_back[i]))
                else:
                    blocks.append(PostRes(self.featureNum_back[i], self.featureNum_back[i]))
        ...
        self.path3 = nn.Sequential(
            ...
            nn.ReLU(inplace = True))
        self.drop = nn.Dropout3d(p = 0.5, inplace = False)
        self.output_cls = nn.Sequential(nn.Conv3d(64, 64, kernel_size = 3, padding = 1),
                                    nn.BatchNorm3d(64),
                                    nn.ReLU(inplace = True),
                                    ...
                                    nn.Sigmoid())
        self.output_reg = nn.Sequential(nn.Conv3d(64, 64, kernel_size = 3, padding = 1),
                                    nn.BatchNorm3d(64),
                                    nn.ReLU(inplace = True),
                                    nn.Dropout3d(p = 0.5, inplace = False),
                                    nn.Conv3d(64,64,kernel_size = 3, padding = 1),
    ...
    def forward(self, x, coord):
        #print(x)
        out = self.preBlock(x) #128*128*128*24
        #print(out.size())
        out_pool,indices0 = self.maxpool1(out) #64*64*64*24
        out1 = self.forw1(out_pool) #64*64*64*32
        out1_pool,indices1 = self.maxpool2(out1) #32*32*32*32
        out2 = self.forw2(out1_pool) #32*32*32*64
        #out2 = self.drop(out2)
        out2_pool,indices2 = self.maxpool3(out2) #16*16*16*64
        out3 = self.forw3(out2_pool) #16*16*16*64
        out3_pool,indices3 = self.maxpool4(out3) #8*8*8*64
        out4 = self.forw4(out3_pool) #8*8*8*64 
        lat3 = self.back3(out3)#16*16*16*64
        rev3 = self.path1(out4)#16*16*16*64
        comb3 = rev3 + lat3 #16*16*16*64
        lat2 = self.back2(torch.cat((out2, coord), 1))#32*32*32*64
        rev2 = self.path2(comb3)#32*32*32*64
        comb2 = rev2 + lat2#32*32*32*64

        lat1 = self.back1(out1) #64*64*64*64
        #print(comb2.size())
        rev1 = self.path3(comb2) #64*64*64*64
        comb1 = lat1 + rev1 #64*64*64*64
        '''
        out1 = self.output(comb1)#64*64*64*5
        size = out1.size()
        out1 = out1.view(out1.size(0), out1.size(1), -1)
        out1 = out1.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], 5)
        out2 = self.output(comb2)#32*32*32*5
        size = out2.size()
        out2 = out2.view(out2.size(0), out2.size(1), -1)
        out2 = out2.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], 5)
        out3 = self.output(comb3)#16*16*16*5
        size = out3.size()
        out3 = out3.view(out3.size(0), out3.size(1), -1)
        out3 = out3.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], 5)
        out4 = self.output(out4) #8*8*8*5
        size = out4.size()
        out4 = out4.view(out1.size(0), out4.size(1), -1)
        out4 = out4.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], 5)
        '''
        out1 = torch.cat((self.output_cls(comb1), self.output_reg(comb1)), dim = 1)
        #print(out1.size())
        size = out1.size()
        #print(size)
        out1 = out1.view(out1.size(0), out1.size(1), -1)
        out1 = out1.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], 5)
        #print(out1)
        out2 = torch.cat((self.output_cls(comb2), self.output_reg(comb2)), dim = 1)
        size = out2.size()
        out2 = out2.view(out2.size(0), out2.size(1), -1)
        out2 = out2.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], 5)
        #print(size)
        #print(out2)
        out3 = torch.cat((self.output_cls(comb3), self.output_reg(comb3)), dim = 1)#16*16*16*5
        size = out3.size()
        out3 = out3.view(out3.size(0), out3.size(1), -1)
        out3 = out3.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], 5)
        #print(out3)
        out4 = torch.cat((self.output_cls(out4), self.output_reg(out4)), dim = 1) #8*8*8*5
        size = out4.size()
        out4 = out4.view(out1.size(0), out4.size(1), -1)
        out4 = out4.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], 5)
        #print(out4)

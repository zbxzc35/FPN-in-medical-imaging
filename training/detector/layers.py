...
'''
def focal_loss(output):
    square = 1 - output 
    square = square * square
    output_log = torch.log(output)
    loss = -0.25 * square * output_log
...
class Loss(nn.Module):
    ...
    def forward(self, output0, label0, output1, label1, output2, label2, output3, label3, train = True):
        #batch_size = labels.size(0)
        output0 = output0.view(-1, 5)
        output1 = output1.view(-1, 5)
        output2 = output2.view(-1, 5)
        output3 = output3.view(-1, 5)
        output = torch.cat((output0, output1, output2, output3), dim = 0)
        label0 = label0.view(-1, 5)
        label1 = label1.view(-1, 5)
        label2 = label2.view(-1, 5)
        label3 = label3.view(-1, 5)
        labels = torch.cat((label0, label1, label2, label3), dim = 0)
        #output = output.view(-1, 5)
        #labels = labels.view(-1, 5)
        ...
        if len(pos_output)>0:
            ...
            regress_losses = [
                ...
                self.regress_loss(pd, ld)]
            regress_losses_data = [l.data[0] for l in regress_losses]
            classify_loss = focal_loss(pos_prob) + focal_loss(neg_prob)
            pos_correct = (pos_prob.data >= 0.5).sum()
            pos_total = len(pos_prob)

        else:
            regress_losses = [0,0,0,0]
            classify_loss =  focal_loss(neg_prob)
            pos_correct = 0
            pos_total = 0
            regress_losses_data = [0,0,0,0]
        classify_loss_data = classify_loss.data[0]
